...
def create_campaign_summary_sheet(model: CustomerModel, writer):
    ...
    summary_data = {
        'Metric': [
            ...
            'Campaign Orders Received',
            'Campaign Revenue Generated',
            'New Customers Acquired',
            'Average Revenue per Order',
            'Campaign Average Revenue per Order',
            'Campaign Start Date',
            'Campaign End Date',
        ...
        'Value': [
            ...
            sum(agent.new_order_count for agent in model.agents if isinstance(agent, CustomerAgent)),
            f"₾{sum(agent.campaign_spending for agent in model.agents if isinstance(agent, CustomerAgent)):,.2f}",
            ...
            f"₾{sum(agent.campaign_spending for agent in model.agents if isinstance(agent, CustomerAgent)) / sum(agent.new_order_count for agent in model.agents if isinstance(agent, CustomerAgent)):,.2f}" if sum(agent.new_order_count for agent in model.agents if isinstance(agent, CustomerAgent)) > 0 else "₾0.00",
    ...
    for agent in model.agents:
        if isinstance(agent, CustomerAgent) and len(agent.prize_wins) > 0:
            prize_winners_data.append({
                ...
                'Campaign Spending': f"₾{agent.campaign_spending:,.2f}",
                'Campaign Average Order Value': f"₾{agent.campaign_spending / agent.new_order_count:,.2f}" if agent.new_order_count > 0 else "₾0.00",
